...
class Friend:
    ...
    def update(self, screen_w, screen_h):
        ...
        elif self.move_type == 'static_fixed': # ミズヒキイカ用（動かない）
            pass
    ...
    def check_light(self, light_x, light_y, light_r):
        ...
        if self.move_type == 'pass' and self.w > 32: 
            if self.is_moving_right:
                ...
                head_x = self.x + 10
        elif self.name == "Mizuhikiika": # ミズヒキイカは縦長なので判定調整
...
class App:
    def __init__(self):
        ...
        self.temp_max = 110
        self.temp_decay_speed = 0.5 # 難易度調整用
        ...
        self.show_collection = False # コレクション画面表示フラグ
        
        # ★英語名マッピング
        self.NAME_MAP = {
            "Mendako": "Pancake Octopus",
            "OwanKurage": "Crystal Jelly",
            "Mizuhikiika": "Bigfin Squid",
            "FusaAnkou": "Sea toad",
            "Demenigisu": "Barreleye",
            "Onikinme": "Fangtooth",
            "Ryugu": "Oarfish",
            "Zarabikunin": "Rough Snailfish",
            "Oumugai": "Nautilus",
            "Ginzame": "Ghost Shark"
        }
        ...
        # ★友達データ定義
        self.FRIEND_TYPES = [
            # 通常種 (weightは出現の重み付けに使用)
            {"name": "Mendako",     "w":16, "h":16, "u":0,  "v":144, "au":16, "av":144, "type":"float_low", "rarity": 10},
            {"name": "Ryugu",       "w":64, "h":16, "u":0,  "v":160, "au":-1, "av":-1,  "type":"pass", "rarity": 10},
            {"name": "Zarabikunin", "w":16, "h":16, "u":0,  "v":176, "au":16, "av":176, "type":"swim", "rarity": 10},
            {"name": "OwanKurage",  "w":16, "h":16, "u":32, "v":176, "au":48, "av":176, "type":"float_mid", "rarity": 10},
            {"name": "Ginzame",     "w":32, "h":16, "u":0,  "v":192, "au":0,  "av":208, "type":"swim", "rarity": 10}, 
            {"name": "Onikinme",    "w":16, "h":16, "u":32, "v":192, "au":48, "av":192, "type":"swim", "rarity": 10},
            {"name": "Oumugai",     "w":16, "h":16, "u":32, "v":208, "au":-1, "av":-1,  "type":"static", "rarity": 10},
            {"name": "FusaAnkou",   "w":16, "h":16, "u":0,  "v":224, "au":-1, "av":-1,  "type":"static_low", "rarity": 10},
            {"name": "Demenigisu",  "w":16, "h":16, "u":32, "v":224, "au":48, "av":224, "type":"swim", "rarity": 10},
            # レア種
            {"name": "Mizuhikiika", "w":16, "h":96, "u":80, "v":24,  "au":-1, "av":-1,  "type":"static_fixed", "rarity": 2}, # rarityを低く設定
        ]
    ...
    # ★クリック/タップが指定領域内か判定するヘルパー
    def is_tap_in_rect(self, x, y, w, h):
    ...
    def update(self):
        ...
        if not self.is_alive:
            ...
            btn_y = self.SCREEN_H // 2 + 35
            
            # コレクション表示切替
            if self.is_tap_in_rect(btn_x, btn_y, btn_w, btn_h) or self.is_cancel_btn_pressed():
                self.show_collection = not self.show_collection

            # リセット処理（コレクション表示中は無効にする）
            if not self.show_collection:
                if (pyxel.btnp(pyxel.KEY_R) or self.is_action_btn_pressed()): 
        ...
        if self.phase == 3:
            ...
            if is_heating: 
                ...
                if self.temp_val > 0: self.temp_val -= self.temp_decay_speed # 難易度で変化
            ...
            if is_release or self.temp_val >= self.temp_max:
                if self.temp_target_min <= self.temp_val <= self.temp_target_max:
                    ...
                    pyxel.play(2, 5)
                    # ★難易度上昇：次回のターゲット範囲を狭く、下降速度を早く
                    difficulty_mod = min(self.spawn_times * 2, 10) # 最大でも10狭める
                    self.temp_target_min = 70 + difficulty_mod
                    self.temp_target_max = 90 - difficulty_mod
                    self.temp_decay_speed = 0.5 + (self.spawn_times * 0.1) # 下降速度アップ
                else:
    ...
    def spawn_friend(self):
        # 候補リスト作成
        candidates = []
        for f in self.FRIEND_TYPES:
            # 条件チェック
            if f["name"] == "Mizuhikiika":
                if self.spawn_times < 4: continue # 4回未満なら候補に入れない
            
            # ウェイト（rarity）分だけ候補に追加する（確率の重み付け）
            for _ in range(f["rarity"]):
                candidates.append(f)
        
        if not candidates: return

        f_data = candidates[pyxel.rndi(0, len(candidates) - 1)]
        ...
        elif f_data["name"] == "Mizuhikiika":
             spawn_y = pyxel.rndi(20, 50) # ミズヒキイカは上の方
    ...
    def reset_game_full(self):
        ...
        self.fry_particles = []
        self.active_friends = [] 
        self.show_collection = False # コレクション閉じる
        # 難易度リセット
        self.temp_target_min = 70
        self.temp_target_max = 90
        self.temp_decay_speed = 0.5
    ...
    def update_foods(self):
        new_foods = []
        fish_center_x = self.fish_x + self.FISH_W / 2
        fish_center_y = self.fish_y + self.FISH_H / 2
        
        for food in self.foods:
            food[1] += 0.5 
            
            # ★接触判定（魚の口元あたり）
            dx = food[0] - fish_center_x
            dy = food[1] - fish_center_y
            hit_range = 10 if self.phase == 1 else 8 # オス融合時は少し広い
            
            if self.is_alive and abs(dx) < hit_range and abs(dy) < hit_range:
                # 食べた時の処理
                self.hunger = min(self.hunger + 10, 100)
                pyxel.play(2, 1) # 音を鳴らす
            elif food[1] < self.SCREEN_H: 
    ...
    def draw(self):
        ...
        if self.is_alive:
            ...
            # コレクションが開いているかどうかで表示を変える
            if self.show_collection:
                # 閉じるボタン
                pyxel.rect(btn_x, btn_y, btn_w, btn_h, 8)
                pyxel.text(btn_x + 25, btn_y + 2, "CLOSE", 7)
                
                # コレクションウィンドウ
                pyxel.rect(10, 10, self.SCREEN_W - 20, self.SCREEN_H - 20, 1)
                pyxel.rectb(10, 10, self.SCREEN_W - 20, self.SCREEN_H - 20, 7)
                pyxel.text(60, 15, "COLLECTION", 7)
                
                y_offset = 30
                
                # 全種類のリストを取得して表示
                all_friends_names = [f["name"] for f in self.FRIEND_TYPES]
                
                for i, name in enumerate(all_friends_names):
                    is_discovered = name in self.discovered_friends
                    col = 10 if is_discovered else 13
                    # 英語名に変換
                    display_name = self.NAME_MAP[name] if is_discovered else "?????????"
                    
                    # 2列表示の計算
                    col_x = 15 if i < 5 else 85
                    row_y = y_offset + (i % 5) * 12
                    
                    # 文字が長い場合に備えて少し小さく描画（Pyxel標準フォントだとサイズ変更できないので色はそのまま）
                    pyxel.text(col_x, row_y, display_name, col)
            else:
                # 開くボタン
                # 点滅演出
                col = 7 if pyxel.frame_count % 30 < 15 else 6
                pyxel.rectb(btn_x, btn_y, btn_w, btn_h, col)
                pyxel.text(btn_x + 8, btn_y + 2, "[ VIEW COLLECTION ]", 7)
                
                # リトライ案内
                pyxel.text(self.SCREEN_W//2 - 35, self.SCREEN_H - 10, "Tap Screen to Retry", 5)
