...
class App:
    def __init__(self):
        ...
        # 2: オス付着（ぷにゅ）- 短形波でビブラート
        pyxel.sound(2).set("c2d2", "s", "4", "v", 8)
        # 3: 水温上昇（ぶいい～ん）- ノイズをスライドさせる
        pyxel.sound(3).set("c1c2", "n", "1", "s", 3)
        # 4: 失敗（ブブー！）- 低い矩形波
        pyxel.sound(4).set("c1r c1", "s", "5", "n", 8)
        # 5: 産卵（ぴろぴろぴ～ん）- 高い音のアルペジオ
        pyxel.sound(5).set("c3e3g3c4", "t", "4", "n", 8)
        
        # 6-7: ゲームオーバーBGM（ドビュッシー「夢」風アレンジ）
        # 非常にゆっくりとしたテンポ(speed 45)で、正弦波(s)を使って優しく
        # メロディ: F -> E -> D -> E -> F -> D -> E ...
        pyxel.sound(6).set(
    ...
    def update(self):
        ...
        if self.is_alive:
            ...
            # =========================================================================
            # ★ステータス減少ロジック（加齢＆吸収による難易度上昇）
            # =========================================================================
            # 基本は60フレーム(2秒)ごと
            base_decay_interval = 60
            
            # ペナルティ1: オスを吸収するごとに間隔が短くなる (1匹につき -3フレーム)
            absorption_penalty = self.total_absorbed_males * 3
            
            # ペナルティ2: 生存時間に応じて間隔が短くなる (20秒=600フレームにつき -1フレーム)
            age_penalty = self.survival_frames // 600
            
            # 現在の減少間隔を計算 (最低でも15フレームは確保＝最速0.5秒)
            ...
            if pyxel.frame_count % current_decay_interval == 0:
                ...
                if self.hunger <= 0 or self.mood <= 0:
                    self.is_alive = False
                    pyxel.playm(0, loop=True) # ★BGMスタート（死亡時）
            # =========================================================================
            ...
            # --- ゲーム進行 ---
            if self.phase == 3:
                # 【スマホ対応】クリック長押し OR ボタン長押し(Z=Aボタン, X=Bボタン)
                is_heating = pyxel.btn(pyxel.MOUSE_BUTTON_LEFT) or pyxel.btn(pyxel.KEY_Z) or pyxel.btn(pyxel.KEY_X)
                is_release = pyxel.btnr(pyxel.MOUSE_BUTTON_LEFT) or pyxel.btnr(pyxel.KEY_Z) or pyxel.btnr(pyxel.KEY_X)
                
                if is_heating: 
                    self.temp_val += 1.5
                    # ★水温上昇中の音（4フレームごとに鳴らして連続音ぽくする）
                ...
                if is_release or self.temp_val >= self.temp_max:
                    if self.temp_target_min <= self.temp_val <= self.temp_target_max:
                        ...
                        pyxel.play(2, 5) # ★産卵成功音（ぴろぴろぴ～ん）
                    else:
                        self.mood = max(0, self.mood - 20); self.temp_val = 0
                        pyxel.play(2, 4) # ★失敗音（ブブー！）
                        if self.mood <= 0: 
                             self.is_alive = False
                             pyxel.playm(0, loop=True) # ★BGMスタート（死亡時）
        ...
        # 【スマホ対応】Pキー(LOVE)をZキー(Aボタン)でも反応させる
        if pyxel.btnp(pyxel.KEY_P) or pyxel.btnp(pyxel.KEY_Z) or pyxel.btnp(pyxel.MOUSE_BUTTON_RIGHT):
            if self.is_alive: 
                self.mood = min(self.mood + 10, 100)
                if self.phase == 0: self.estrous = min(self.estrous + 2, self.ESTROUS_MAX)
                self.happy_timer = 90
                pyxel.play(2, 0) # ★LOVE音（ポワン）
        ...
        if not self.is_alive:
            ...
            # 【スマホ対応】リセットをZ/Xキー(A/Bボタン)でも反応させる
            if pyxel.btnp(pyxel.KEY_R) or pyxel.btnp(pyxel.KEY_Z) or pyxel.btnp(pyxel.KEY_X): 
    ...
    def draw(self):
        ...
        if self.is_alive:
            ...
            # 【スマホ対応】LキーをXキー(Bボタン)でも反応させる
            if pyxel.btn(pyxel.KEY_L) or pyxel.btn(pyxel.KEY_X):
